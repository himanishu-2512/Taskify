...
const ShareButton = ({access}) => {
  ...
  const [permission, setPermission] = useState<"can edit" | "can read"|"no access">("can edit");
  ...
  const handlePermissionChange = (newPermission: "can edit" | "can view"|"no access") => {
    setPermission(newPermission);
  };
  const addPermission=async()=>{
try {
  const response=await postRequest(API_ENDPOINTS.DOCUMENT.ADD_PERMISSIONS(docsId),{},{},{targetUserEmail:email, accessLevel:permission},token);
  ...
  const togglePublicPrivate = async() => {
    try {
      const response=await postRequest(API_ENDPOINTS.DOCUMENT.CHANGE_VISIBILITY(docsId),{},{},{visibility:isPublic?"private":"public"},token)
      if(response?.data?.message){
        alert(response?.data?.message)
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="primary">Share<CiShare2/></Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-md backdrop:blur-md">
        ...
        <div className="mb-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild className="ml-[10px]">
              <Button variant="tertiary">{permission === "can edit" ? "can edit" :permission==="can read"? "can read":"no access"}<RiArrowDropDownLine/></Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem onClick={() => handlePermissionChange("can edit")}>Can Edit</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handlePermissionChange("can read")}>Can Read</DropdownMenuItem>
              <DropdownMenuItem onClick={() => handlePermissionChange("no access")}>Revoke access</DropdownMenuItem>
        ...
        <DialogFooter>
          <Button type="button" variant="primary" onClick={()=>addPermission()}>
            Add Permission
          </Button>
        </DialogFooter>
      </DialogContent>
...
const Page = () => {
      ...
      try {
        ...
        const fixedContent = {
          blocks: Object.values(version.blockMap).map((block: any) => ({
            ...
            text: block.text,
            depth: block.depth,
            inlineStyleRanges: block.characterList.map((char: any, index: number) => ({
              offset: index,
              length: 1,
  ...
  const fetchDocumentDeatil=async()=>{
    try {
      const response=await getRequest(API_ENDPOINTS.DOCUMENT.GET_DETAIL(docsId),{},{},token);
    console.log(response)
    setDocumentTitle(response?.data?.document?.title)
    setDocumentDescription(response?.data?.document?.description)
    if(response?.data?.document?.latestContent&&response?.data?.document?.latestContent!=''){
  ...
  const fetchDocumentVersions=async()=>{
    console.log("yes")
    const response=await getRequest(API_ENDPOINTS.DOCUMENT.GET_LAST_TEN_VERSIONS(docsId),{},{},token)
    console.log(response)
  }
  ...
  const saveVersion = async() => {
    ...
    const data={title:documentTitle,description:documentDescription,content:contentState}
    const response=await postRequest(API_ENDPOINTS.DOCUMENT.UPDATE_DOCUMENT(docsId),{},{},data,token);
    setVersionHistory(response.data.document.versions);
  ...
  return (
    <div style={{ padding: "20px",minWidth:"800px", maxWidth: "800px", margin: "auto" }}>
      ...
      <div style={{ marginTop: "20px" }}>
        <Button onClick={saveVersion}><IoMdSave/> Save Version</Button>
      </div>

      {/* Version History Dropdown */}
      <div style={{ marginTop: "20px" }}>
        <DropdownMenu>
          <DropdownMenuTrigger className="bg-blue-600 text-white font-semibold rounded-md px-[10px] py-[8px] flex justify-center items-center" onClick={()=>fetchDocumentVersions()}>
      ...
      <div className="mt-[20px]"><ShareButton access={access} /></div>
      </div>
